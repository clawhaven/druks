...
Caddy additionally trusts an identity header inserted by exe.dev or another
upstream proxy — any nonempty value passes the edge, and the backend treats
that identity as authoritative for account resolution. Public webhook routes
and the token-authenticated notification respond route bypass the session gate
but keep their own authentication. A local dashboard bound directly to
