...
// A 401 is not a message to display — it means the session is gone. Typed so
// callers can branch, and broadcast so the AuthProvider unmounts the app.
...
// Session identity rides the cookie on every call — same-origin, explicit.
...
// The account door: harness login mints the session; both the landing screen
// and Settings reconnect drive this same surface.
