...
describe('HarnessConnect', () => {
  it('shows the signed-in identity, seat identity, mismatch, and default state', () => {
    renderCard(
      harness({
        connected: true,
        account: 'ops@corp.com',
        providerEmail: 'personal@example.com',
        isEmailMismatch: true,
        isDefault: true,
      }),
    )
    expect(screen.getByText('connected · ops@corp.com')).toBeTruthy()
    expect(screen.getByText('default seat')).toBeTruthy()
    expect(screen.getByText(/personal@example.com differs from your sign-in/)).toBeTruthy()
  ...
  it('surfaces a rejected sign-in inline', async () => {
    const fetchMock = vi.fn(async (url: string) => {
      if (url.endsWith('/login/start')) {
        return new Response(JSON.stringify({ authorizeUrl: 'https://x', loginId: 'L1' }), {
          status: 200,
        })
      }
      return new Response(JSON.stringify({ error: 'HTTP_409', detail: 'seat already connected' }), {
        status: 409,
      })
    })
    vi.stubGlobal('fetch', fetchMock)
    ...
    expect(screen.getByText('seat already connected')).toBeTruthy()
  })
})
