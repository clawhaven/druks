...
// One harness's subscription connection: the signed-in account's own seat —
// status chip + the shared /api/auth connect flow + account-scoped disconnect.
// Connection state is its own axis — persisted immediately, not through the
// modal's Save — so this manages its own busy/error and refetches the
// harnesses query on change.
  ...
  return (
    <div className="hr-connect">
      <div className="hr-conn-status">
        ...
        {harness.connected && harness.isDefault && (
          <span className="hr-chip hr-chip-off" title="Runs use this seat while execution isn't seat-aware.">
            default seat
          </span>
        )}
      ...
      {harness.connected && harness.providerEmail && harness.isEmailMismatch && (
        <div className="hr-conn-exp">
          provider account {harness.providerEmail} differs from your sign-in — runs still use this
          seat.
        </div>
      )}
