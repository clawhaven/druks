...
- **Un-runnable mandatory verification.** Before promoting a verification command (test suite, production build, typecheck) to mandatory, consider whether it can even run in the sandbox: right runtime major (Node/Python), deps installed, no private-registry or network it lacks. If you can't be confident it executes, frame it as "run and report results if the command executes; otherwise report the exact command and blocker as not_run" — never a hard gate the box provably cannot run. A mandatory check the sandbox can't execute blocks the PR forever with no path forward.
