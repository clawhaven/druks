...
def test_harness_response_carries_connection_state(tmp_path: Path):
    ...
    assert claude["providerEmail"] is None
    assert claude["isEmailMismatch"] is False
    assert claude["isDefault"] is False
    assert "expiresAt" in claude


def test_harnesses_show_only_the_signed_in_accounts_seat(tmp_path: Path, db_session):
    from conftest import connect_harness
    from druks.harnesses.claude import ClaudeHarness

    # The suite's session gate stands in op@example.com; a seat on another
    # account never shows on this card.
...
def test_harness_card_reports_mismatch_and_default_state(tmp_path: Path, db_session):
    from druks.accounts.models import Account
    from druks.harnesses.models import HarnessLogin

    # The signed-in account's seat carries a different provider identity —
    # visible as a mismatch, never a block.
...
def test_disconnect_removes_only_the_signed_in_accounts_seat(tmp_path: Path, db_session):
    from conftest import connect_harness
    from druks.harnesses.claude import ClaudeHarness
    from druks.harnesses.models import HarnessLogin
    ...
    other = connect_harness(
        ClaudeHarness, {"claudeAiOauth": {"accessToken": "y"}},
        provider_email="someone-else@example.com",
    )
    ...
    assert HarnessLogin.reload(mine_id) is None
    assert HarnessLogin.reload(other_id) is not None


def test_disconnect_without_a_seat_is_a_no_op(tmp_path: Path):
    with _build_client(tmp_path) as client:
        response = client.delete("/api/settings/harnesses/claude/login")
    assert response.status_code == 200
    assert response.json()["connected"] is False
